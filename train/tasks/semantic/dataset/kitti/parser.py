...
def my_collate(batch):
    ...
    for k in to_augment_unique:
        ...
        proj_labels = torch.cat(
            (proj_labels, torch.flip(proj_labels[k.item()], [1]).unsqueeze(0)),
            dim=0,
        )
        project_mask = torch.cat(
            (
                project_mask,
                torch.flip(project_mask[k.item()], [1]).unsqueeze(0),
            ),
            dim=0,
        )
...
class SemanticKitti(Dataset):
    def __init__(
        ...
        if os.path.isdir(self.root):
            print(
                "Sequences folder exists! Using sequences from %s" % self.root
            )
        else:
            raise ValueError(
                f"Sequences folder '{self.root}' doesn't exist! Exiting..."
            )
    ...
    def __getitem__(self, index):
        ...
        proj = (
            proj - self.sensor_img_means[:, None, None]
